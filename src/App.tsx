...
function Scanner({ isProcessing, onCapture }: { isProcessing: boolean, onCapture: (data: string) => void }) {
  ...
  const processAndResizeImage = (dataUrl: string) => {
    const img = new Image();
    img.onload = () => {
      ...
      if (ctx) {
        ...
        // compress slightly to save API payload size and network time
    ...
    img.onerror = () => {
       onCapture(dataUrl); // fallback if parsing fails
    };
    img.src = dataUrl;
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        stopCamera();
        processAndResizeImage(event.target.result as string);
      }
    };
